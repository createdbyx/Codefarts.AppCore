...
namespace Codefarts.AppCore.Unity.Editor
{
    ...
    public class SubmitFeedbackWindow : EditorWindow
    {
        ...
        public static void ShowFeedbackWindow()
        {
            ...
            GetWindow<SubmitFeedbackWindow>(local.GetString("Feedback"), true);
        }
        ...
        public void OnGUI()
        {
            ...
            {
                // draw email field
                GUILayout.Label(local.GetString("EmailOptional"));
                this.email = GUILayout.TextField(this.email);

                // draw subject field
                GUILayout.Label(local.GetString("Subject"));
                this.subject = GUILayout.TextField(this.subject);

                // draw message area
                GUILayout.Label(local.GetString("Message"));
                ...
                if (GUILayout.Button(local.GetString("Submit")))
                {
                    // set submitting state and set initial status message
                    this.isSubmitting = true;
                    this.statusMessage = local.GetString("Submitting");
                    ...
                    else
                    {
                        ...
                        this.errorMessage = local.GetString("ERR_CouldNotParseUri");
                    }
                }

                // add some flexible space to separate the buttons and draw the close button.
                GUILayout.FlexibleSpace();
                if (GUILayout.Button(local.GetString("Close")))
                {
        ...
        public void Update()
        {
            ...
            if (this.successfullySubmitted)
            {
                ...
                EditorUtility.DisplayDialog(local.GetString("Success"), this.statusMessage, local.GetString("Close"));
        ...
        private void UploadValuesCompleted(object sender, UploadValuesCompletedEventArgs e)
        {
            ...
            if (e.Cancelled)
            {
                this.errorMessage = local.GetString("Canceled");
                return;
            }
