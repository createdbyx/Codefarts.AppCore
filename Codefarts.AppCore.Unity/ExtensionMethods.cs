...
namespace Codefarts.AppCore.Unity
{
    ...
    public static class ExtensionMethods
    {
        ...
        /// </returns>
        public static Color32 GetColorSetting(this ISettingsProvider values, string name, Color32 defaultValue)
        {
            // set default value
            var color = defaultValue;

            // check if setting exists
            // if (values.TryGetSetting(name, out color)) // values.HasValue(name))
            // {
        ...
        /// </param>
        public static void SetColorSetting(this ISettingsProvider values, string name, Color32 color)
        {
            // convert to string
            var value = string.Format("{0} {1} {2} {3}", color.a, color.r, color.g, color.b);

            // store setting
            values.SetSetting(name, value);
        }
    }
}
